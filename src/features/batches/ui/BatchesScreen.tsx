...
const TIME_OPTIONS = ["05:00", "06:00", "07:00", "08:00", "09:00", "10:00", "11:00", "12:00", "13:00", "14:00", "15:00", "16:00", "17:00", "18:00", "19:00", "20:00", "21:00", "22:00", "23:00", "00:00", "01:00", "02:00"];
...
export function BatchesScreen() {
	...
	const renderTimePicker = () => (
		<View style={styles.timePickerContainer}>
			<Text style={styles.timePickerTitle}>Select {selectingTimeFor === "start" ? "Start" : "End"} Time</Text>
			<ScrollView horizontal={false} showsVerticalScrollIndicator={false} contentContainerStyle={styles.timePickerGrid}>
			...
			<ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
				...
				{isLoading ? (
					...
					<View style={styles.emptyContainer}>
						<Ionicons name="calendar-outline" size={64} color={colors.mutedForeground} style={{ opacity: 0.3 }} />
						<Text style={styles.emptyText}>No batches available yet.</Text>
						<Text style={styles.emptySubText}>Tap the + icon in the header to create your first timeslot.</Text>
					</View>
				) : (
					batches.map((batch) => (
						...
						{selectingTimeFor ? (
							...
							<>
								<View style={styles.modalHeader}>
									...
									<Pressable accessibilityRole="button" style={styles.modalCloseButton} onPress={() => setIsAddModalVisible(false)}>
										<Ionicons name="close" size={24} color={colors.primary} />
									</Pressable>
								</View>

								<Text style={styles.inputLabel}>Batch Name</Text>
								<TextInput style={styles.modalInput} placeholder="e.g. Morning Focus" value={newName} onChangeText={setNewName} placeholderTextColor="rgba(0,0,0,0.3)" />

								<View style={styles.modalRowGroup}>
									<View style={{ flex: 1 }}>
										<Text style={styles.inputLabel}>Start Time</Text>
										<Pressable style={styles.modalTimeSelector} onPress={() => setSelectingTimeFor("start")}>
											<Ionicons name="time-outline" size={20} color={colors.primary} style={{ marginRight: 8 }} />
											<Text style={[styles.modalTimeSelectorText, !newStartTime && { color: "rgba(0,0,0,0.3)" }]}>{formatTimeDisplay(newStartTime)}</Text>
										</Pressable>
									</View>
									<View style={{ flex: 1 }}>
										<Text style={styles.inputLabel}>End Time</Text>
										<Pressable style={styles.modalTimeSelector} onPress={() => setSelectingTimeFor("end")}>
											<Ionicons name="time-outline" size={20} color={colors.primary} style={{ marginRight: 8 }} />
											<Text style={[styles.modalTimeSelectorText, !newEndTime && { color: "rgba(0,0,0,0.3)" }]}>{formatTimeDisplay(newEndTime)}</Text>
										</Pressable>
									</View>
								</View>

								<Text style={styles.inputLabel}>Monthly Fee (Rs)</Text>
								<TextInput style={styles.modalInput} placeholder="500" value={newFee} onChangeText={setNewFee} keyboardType="numeric" placeholderTextColor="rgba(0,0,0,0.3)" />
								...
								<Pressable style={[styles.modalSubmitBtn, isSubmitting && { opacity: 0.7 }]} onPress={handleCreateBatch} disabled={isSubmitting}>
									{isSubmitting ? <ActivityIndicator color="#fff" size="small" /> : <Text style={styles.modalSubmitText}>Create Batch</Text>}
								</Pressable>
							</>
						)}
					</View>
